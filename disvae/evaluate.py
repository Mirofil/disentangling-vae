...
class Evaluator:
    ...
    def compute_metrics(self, dataloader, dataset=None):
        ...
        if dataset in ['dsprites', 'mpi3dtoy', '3dshapes']: # Most metrics are only applicable for datasets with ground truth variation factors
            ...
            accuracies = self._disentanglement_metric(dataloader.dataset, method_names, sample_size=int(self.sample_size), n_epochs=10000, dataset_size=int(self.dataset_size), hidden_dim=512, use_non_linear=False)
